    ...
    <>
      ...
      <div className="content-holder">
        ...
        <div className="category-holder">
          {eachArticle
            .filter((item) => (item.category === "Entertainment" ? item : !item))
            .reverse()
            // .slice(0, 15)
            .map((item) => (
              <div className="each-article" key={item._id}>
                <Link to={`/dashboard/blog/current-post/${item._id}`} className="link">
                  {item.title} <span className="article-span" style={{fontSize: "small", color: "purple"}}>
                  {new Date(item.updatedAt).getDay()}.
                  {new Date(item.updatedAt).getMonth() + 1}.
                  {new Date(item.updatedAt).getFullYear()}
                  </span>
                  <span className="article-span-author" style={{fontSize: "small", color: "purple"}}>by - {item.author}</span>
                </Link>
              </div>
            ))}
